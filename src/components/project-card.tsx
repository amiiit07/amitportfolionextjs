...
export function ProjectCard({ project, delay = 0 }: ProjectCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative rounded-[1.8rem] bg-[#0c1429]/72 p-5 backdrop-blur-xl sm:p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-4">
            {project.icon && (
              <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-accent/20 to-accent-2/20 text-2xl shadow-lg sm:h-14 sm:w-14 sm:text-3xl">
                {project.icon}
              </span>
            )}
            <div>
              <h3 className="font-display text-lg font-bold text-white transition-colors group-hover:text-[#a5b4fc] sm:text-xl">
                {project.title}
              </h3>
        ...
        <div className="mt-6 flex flex-col gap-2.5 border-t border-white/10 pt-5 sm:flex-row sm:flex-wrap sm:gap-3">
          {project.live_url ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="group/btn inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#3b82f6]/48 bg-[#3b82f6]/18 px-4 py-2 text-sm font-semibold text-[#dbeafe] transition-all hover:bg-[#3b82f6] hover:text-white sm:w-auto"
            >
          ...
          {project.repo_url ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-white/20 bg-white/5 px-4 py-2 text-sm font-semibold text-white/80 transition-all hover:border-white/40 hover:bg-white/10 sm:w-auto"
            >
